...
import jw.fluent_api.desing_patterns.unit_of_work.api.Repository;
...
public class RepositoryBase<T extends DataModel> implements Repository<T,UUID>, CustomFile
{
    ...
    public boolean load() {
        try {
            ...
            FluentLogger.error("Repository load error "+fileName + " " + entityClass.getName(), e);
            return false;
        }
    }
    ...
    public boolean save() {
        try {
            ...
            FluentLogger.error("Repository save error "+fileName + " " + entityClass.getName(), e);
            return false;
        }
    }
}
